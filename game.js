...
function hit(index) {
    ...
    if (index == 1) {
        if (player_batting1 < team1.players.length) {
            if (run == 0) {
                document.getElementById("score1").innerHTML = `0<br>Player-${player_batting1 + 1} OUT`;
            }
            else {
                document.getElementById("score1").innerHTML = `Player-${player_batting1 + 1} Scored<br>${run}`;
            }
            team1.players[player_batting1].score.push(run);
            player_total1 += run;
            team1.total += run;
            (document.getElementById(`B-${index}${player_batting1 + 1}${balls + 1}`)).innerHTML = String(run);
            (document.getElementById(`total${player_batting1 + 1}${index}`)).innerHTML = String(player_total1);
            balls += 1;
            if (balls == 6 || run == 0) {
        ...
        if (player_batting2 < team2.players.length) {
            if (run == 0) {
                document.getElementById("score2").innerHTML = `0<br>Player-${player_batting2 + 1} OUT`;
            }
            else {
                document.getElementById("score2").innerHTML = `Player-${player_batting2 + 1} Scored<br>${run}`;
            }
            team2.players[player_batting2].score.push(run);
            player_total2 += run;
            team2.total += run;
            (document.getElementById(`B-${index}${player_batting2 + 1}${balls + 1}`)).innerHTML = String(run);
            (document.getElementById(`total${player_batting2 + 1}${index}`)).innerHTML = String(player_total2);
            balls += 1;
            if (balls == 6 || run == 0) {
        ...
        for (let j = 0; j < 8; j++) {
            if (i == 0) {
                if (j == 0) {
                    ...
                    (document.getElementById(`thead${index}`)).innerHTML += `<th scope="col">Ball-${j}</th>`;
                }
            }
        }
